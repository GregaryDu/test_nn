...
#				self.middle_res['delt_B'][layer_num]      = np.mean(self.middle_res['layer_delt'][layer_num+1], axis=0)
				self.middle_res['delt_W'][layer_num]      = \
					np.dot(self.middle_res['layer_prob'][layer_num].T, self.middle_res['layer_delt'][layer_num+1])#/self.batch_size
				self.middle_res['delt_B'][layer_num]      = np.sum(self.middle_res['layer_delt'][layer_num+1], axis=0)
	...
	def my_nn(self):
		...
		for i in xrange(iter_num):
			...
			# 1) compute predict-y
			layer_2_i = np.dot(batch_x, W_1) + B_1
			layer_2_o = self.sigmoid(layer_2_i)
			layer_2_o_daoshu = self.delt_sigmoid(layer_2_i)		

			layer_3_i = np.dot(layer_2_o, W_2) + B_2
			layer_3_o = self.sigmoid(layer_3_i)
			layer_3_o_daoshu = self.delt_sigmoid(layer_3_i)

			output_i = np.dot(layer_3_o, W_3) + B_3
			output_o = self.sigmoid(output_i)
			output_o_daoshu = self.delt_sigmoid(output_i)

			self.forward_propagate(batch_x)
			self.compute_diff(layer_2_i, self.middle_res['layer_input'][1])
			self.compute_diff(layer_2_o, self.middle_res['layer_prob'][1])
			self.compute_diff(layer_3_i, self.middle_res['layer_input'][2])
			self.compute_diff(layer_3_o, self.middle_res['layer_prob'][2])
			self.compute_diff(output_i , self.middle_res['layer_input'][3])
			self.compute_diff(output_o , self.middle_res['layer_prob'][3])
			# 2) compute delta-each-layer
			output_wucha = output_o - batch_y
			output_delt  = output_wucha * output_o_daoshu
			
			layer_3_wucha= np.dot(output_delt, W_3.T)
			layer_3_delt = layer_3_wucha * layer_3_o_daoshu
			
			layer_2_wucha= np.dot(layer_3_delt, W_2.T)
			layer_2_delt = layer_2_wucha * layer_2_o_daoshu

			delt_W_3 = np.dot(layer_3_o.T , output_delt)#/batch_x.shape[0]
			#delt_B_3 = np.mean(output_delt, axis=0)
			delt_B_3 = np.sum(output_delt, axis=0)
			delt_W_2 = np.dot(layer_2_o.T , layer_3_delt)#/batch_x.shape[0]
			#delt_B_2 = np.mean(layer_3_delt, axis=0)
			delt_B_2 = np.sum(layer_3_delt, axis=0)
			delt_W_1 = np.dot(batch_x.T    , layer_2_delt)#/batch_x.shape[0]
			#delt_B_1 = np.mean(layer_2_delt, axis=0)
			delt_B_1 = np.sum(layer_2_delt, axis=0)

			self.backward_propagate(batch_y)
			self.compute_diff(output_wucha, self.middle_res['layer_wucha'][3])
			self.compute_diff(output_delt , self.middle_res['layer_delt'][3])
			self.compute_diff(layer_3_wucha, self.middle_res['layer_wucha'][2])
			self.compute_diff(layer_3_delt , self.middle_res['layer_delt'][2])
			self.compute_diff(layer_2_wucha, self.middle_res['layer_wucha'][1])
			self.compute_diff(layer_2_delt , self.middle_res['layer_delt'][1])
			# 3) update  the par-w-B
			W_1 -= self.lr*delt_W_1
			B_1 -= self.lr*delt_B_1
			W_2 -= self.lr*delt_W_2
			B_2 -= self.lr*delt_B_2
			self.update_weight()
			self.compute_diff(W_3, self.W[2])
			self.compute_diff(B_3, self.B[2])
			self.compute_diff(W_2, self.W[1])
			self.compute_diff(W_1, self.W[0])
			self.compute_diff(B_2, self.B[1])
			self.compute_diff(B_1, self.B[0])
...
if __name__=='__main__':
	#CTest = CMyNN(hidden_nodes_list=[100], batch_size=100, epoch=1000, lr=0.5)
	#CTest.my_nn()
